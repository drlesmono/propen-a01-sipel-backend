...
import {Card, Input, Typography} from "@material-ui/core";
...
class Laporan extends Component {
    constructor(props) {
        ...
        this.state = {
            laporanList: [
                //{"idReport":1,"reportName":"BAST/PI/23/04/2021","uploadedDate":"2021-04-22T17:00:00.000+00:00","statusApproval":"Pending","reportType":"BAST","signed":false},
                //{"idReport":2,"reportName":"PI/LapInstallasi/2021","uploadedDate":"2021-04-23T12:29:15.000+00:00","statusApproval":"Pending","reportType":"Installation Report","signed":false}
                ],
            orderList:[],
            bastList:[],
            piList:[],
    ...
    async handleAccept(event){
        ...
        const report = this.state.laporanTarget
        ...
        try{
            ...
            APIConfig.put(`/laporan/accept/${this.state.laporanTarget.idReport}`, report);
            console.log(report);
        }catch (error) {
            alert("Oops terjadi masalah pada server");
    ...
    async handleReject(event){
        ...
        try{
            ...
            APIConfig.put(`/laporan/reject/${this.state.laporanTarget.idReport}`, report);
        }catch (error) {
            alert("Oops terjadi masalah pada server");
            console.log(error);
        }
    ...
    async handleDownload(laporan){
        ...
        doc.addImage(img, 'png', startX, startY+10, 25, 13.5);
        doc.setFontSize(15);
        doc.text('FORM BERITA ACARA SERAH TERIMA', startX+40, startY+18.5);
        doc.setFont('times new roman', 'normal', 'normal');
        doc.setFontSize(11);
        doc.text('Nomor Permintaan: '+ bastNum, startX+1, startY+34.5)
        doc.text('Pada hari ini, '+ dateHandover, startX+1,startY+41.5 )
        ...
        if(tipe === "mn"){
            doc.text('Periode: '+ start + ' - ' + end, startX+1, startY+121.5)
            doc.text('No PO: '+ po, startX+1, startY+127.5)
            doc.setFont('times new roman', 'bold', 'bold');

            //border ttd
            doc.line(startX+20, startY+133, startX+20, startY+185);
            doc.line(startX+20, startY+133, startX+155, startY+133);
            doc.line(startX+155, startY+133, startX+155, startY+185);
            doc.line(startX+20, startY+185, startX+155, startY+185);
            doc.line(startX+87.5, startY+133, startX+87.5, startY+185);
            //doc.line(startX+20, startY+128.5, startX+20, startY+131.5);

            doc.text('PIHAK PERTAMA', startX+35, startY+139.5)
            doc.text('PIHAK KEDUA', startX+110, startY+139.5)
            doc.text('('+picName+')', startX+32, startY+179.5)
            doc.text('('+namaKedua+')', startX+117, startY+179.5)

            doc.text('MENGETAHUI',startX+73, startY+194.5)
        ...
        //doc.text('PIHAK PERTAMA')
        //doc.text('PIHAK KEDUA')
        //doc.text('Moh. Farid Taufiqurrahman')
        //doc.text(picKedua)
        //doc.table(startX, startY+15,
    ...
    render() {
        const {laporanList, isLoading, isEdit, laporanTarget, isChange, orderList, bastList, piList, mnList, isPreview} = this.state;
        const tableHeaders = ['No','Id Report', 'Report Type', 'Report Name', 'Uploaded Date', 'Status', 'Action'];
        let sortedLaporanlist;
        if(this.state.laporanTarget !== null){
            let status = laporanTarget.statusApproval;
        }

        const tableRows = laporanList.map((laporan) =>
            [laporan.idReport, laporan.reportType, laporan.reportName, laporan.uploadedDate.substr(0, 10), laporan.statusApproval,
                laporan.statusApproval === "Approved"?
                [<CustomizedButtons variant="contained" size="small" color="#FD693E" onClick={() => this.handleEdit(laporan)}>Ubah Status</CustomizedButtons>,
                    <CustomizedButtons variant="contained" size="small" color="#FD693E" onClick={() => this.handlePreview(laporan)}>Preview</CustomizedButtons>,
                    <CustomizedButtons variant="contained" size="small" color="#FD693E" onClick={() => this.handleDownload(laporan)}>Unduh</CustomizedButtons>]
                    :[<CustomizedButtons variant="contained" size="small" color="#FD693E" onClick={() => this.handleEdit(laporan)}>Ubah Status</CustomizedButtons>,
                        <CustomizedButtons variant="contained" size="small" color="#FD693E" onClick={() => this.handlePreview(laporan)}>Preview</CustomizedButtons>,
                    ]
                ,
        ...
        if(this.state.laporanTarget !== null){
            tipe = laporanTarget.reportType;
            nomor = laporanTarget.reportName;
            status = laporanTarget.statusApproval;
            id = laporanTarget.idReport;
        }
        for(let i=0; i<bastList.length; i++){
            if(bastList[i].idReport === id){
                bast = bastList[i];
        ...
        return(
            <div id="all">
                {isPreview === false ?
                    <div id="content">
                        <CustomizedTables headers={tableHeaders} rows={tableRows} id="tabel" />
                        <Modal show={isEdit && !isChange} handleCloseModal={this.handleCancel}>
                            <h2 style={{ marginTop: 0, marginBottom: 0}}><b>Ubah Status Laporan</b></h2>
                            <table>
                                <tr>
                                ...
                                <tr>
                                    <td><p style={{ marginTop: 0, marginBottom: 0, color:"midnightblue", fontWeight:"bolder", fontSize:"medium" }}>Status Laporan </p></td>
                                    <td><p style={{ marginTop: 0, marginBottom: 0, color: "coral", fontWeight:"bolder", fontSize:"medium" }}>: {status}</p></td>
                                </tr>
                                <tr>
                                    <td><br></br></td>
                                    <td><br></br></td>
                                ...
                                <tr>
                                    <td><CustomizedButtons variant="contained" size="small" color="#2F3F58" onClick={(event)=>this.handleAccept(event)}>Setujui</CustomizedButtons></td>
                                    <td><CustomizedButtons variant="contained" size="small"  color="#FF7F50" onClick={(event)=>this.handleReject(event)}>Tolak</CustomizedButtons></td>
                                </tr>
                            </table>
                        </Modal>
                        <Modal show={isChange} handleCloseModal={this.handleCancel}>
                            <a onClick={(event)=>this.handleClose(event)} href={"/laporan/list"}><h6 id="highlighted2"> &#8810; Kembali ke Daftar Laporan</h6></a>

                            <h4>Status Laporan dengan nomor <p id="highlighted">{nomor}</p>berhasil diubah</h4>
                        </Modal>
                    ...
                    <div id="preview">
                        <Card id="card-preview">
                            <a onClick={(event)=>this.handleClose(event)} href={"/laporan/list"}><p id="highlighted3">    &#8810; Kembali ke Daftar Laporan    </p></a>
                            <Table>
                                <tr>

                                </tr>
                                <tr>
                                    <td colSpan={2}><h2 id="title">Preview Berita Acara Serah Terima</h2></td>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                    <td id="nameFill">{bastNum}</td>
                                    <td id="nameFill">{dateHandover}</td>
                                </tr>
                                <tr>
                                    <td hidden={true}>-----</td>
                                    <td hidden={true}>-----</td>
                                ...
                                <tr>
                                    <td id="nameFill">{picName}</td>
                                    <td id="nameFill">{namaKedua}</td>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                    <td id="nameFill">PT Lintas Media Danawa</td>
                                    <td id="nameFill">{organisasiKedua}</td>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                    <td id="nameFill">Managed Services</td>
                                    <td id="nameFill">{divisiKedua}</td>
                                </tr>
